import sys
from rich import print
from time import sleep
import time

def printLyrics():
    lines = [
        ("I wanna da-", 0.06), #1
        ("I wanna dance in the lights", 0.05), #2
        ("I wanna ro-", 0.08), #3
        ("I wanna rock your body", 0.068), #3
        ("I wanna go", 0.08), #5
        ("I wanna go for a ride", 0.068), #6
        ("Hop in the music and", 0.07), #7
        ("Rock your body", 0.08), #8
        ("Rock that body", 0.069), #9
        ("come on, come on", 0.035), #10
        ("Rock that body", 0.05), #11
        ("(Rock your body)", 0.03), #12
        ("Rock that body", 0.049), #13
        ("come on, come on", 0.35), #14
        ("Rock that body", 0.08), #15

    ]




















    start_time = pygame.time.get_ticks() / 1000  # en secondes

    for line, timestamp in lines:
        while (pygame.time.get_ticks() / 1000 - start_time) < timestamp:
            sleep(0.01)
    print(f"[bold cyan]{line}[/bold cyan]", flush=True)